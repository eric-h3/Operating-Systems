...
MODULE_DESCRIPTION("Extremely powerful Module that intercepts syscalls to do some useful logging");
MODULE_AUTHOR("Ahmad Alsabbagh, Eric Hua");
MODULE_LICENSE("GPL");
...
{
	// call del methods on current->pid
	//spin lock here because we're accessing table values
...
asmlinkage long interceptor(struct pt_regs reg) {
	// if syscall is being monitored for ALL (table[syscall].monitored == 2), then log
	// if syscall is being monitored for current->pid then log (check_pid_monitored(reg.ax, current->pid) == 1)
	// spin lock here because we're accessing table metadata values
	spin_lock(&my_table_lock);
	if (table[reg.ax].monitored == 2 || check_pid_monitored(reg.ax, current->pid) == 1) {
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}
	spin_unlock(&my_table_lock);
	// call original method given reg
	return table[reg.ax].f(reg);
}

asmlinkage long request_start_monitoring(int pid, int syscall) {
	int ret = 0;
	spin_lock(&my_table_lock);
	if (pid == 0) {
		// all pids -> set monitored to 2
		...
		// table[syscall].listcount = NR_syscalls;
	} else {
		// add pid to monitored syscall pids
...
asmlinkage long request_stop_monitoring(int pid, int syscall) {
	int ret = 0;
	spin_lock(&my_table_lock);
	if (pid == 0) {
		// all pids -> destroy list
...
asmlinkage long request_syscall_intercept(int syscall) {
...
asmlinkage long request_syscall_release(int syscall) {
	spin_lock(&sys_call_table_lock);
	spin_lock(&my_table_lock);
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// invalid syscall if larger than limit or less than 0 or is MY_CUSTOM_SYSCALL
	if (syscall >= NR_syscalls || syscall < 0 || syscall == MY_CUSTOM_SYSCALL) {
		return -EINVAL;
	}
	// invalid pids (less than 0 OR pid doesn't belong to a valid process and is not 0)
	if (pid != 0 && (pid < 0 || (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL && current_uid() != 0))) {
		return -EINVAL;
	}
	// if pid is 0 (all pids) and calling process is not 0 then invalid permissions
	if (pid == 0 && current_uid() != 0) {
		return -EPERM;
	}
	// intercepting a syscall without root permissions is invalid
	...
	if ((cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) && current_uid() != 0) {
		...
		if (check_pids_same_owner(pid, current->pid) == -EPERM) {
			return -EPERM;
		}
	}

	// releasing something that's not even intercepted is invalid
	if (cmd == REQUEST_SYSCALL_RELEASE && table[syscall].intercepted == 0) {
		return -EINVAL;
	}
	// stop monitoring something that's not even intercepted or isnt even being monitored is invalid
	if (cmd == REQUEST_STOP_MONITORING && (check_pid_monitored(syscall, (pid_t) pid) == 0 || table[syscall].intercepted == 0)) {
		return -EINVAL;
	}
	// start monitoring something that's not even intercepted is invalid
	if (cmd == REQUEST_START_MONITORING && table[syscall].intercepted == 0) {
		return -EINVAL;
	}
	// intercepting something that's already being intercepted is invalid
	if (cmd == REQUEST_SYSCALL_INTERCEPT && table[syscall].intercepted == 1) {
		return -EBUSY;
	}
	// monitoring something that's already being monitored is invalid
	if (cmd == REQUEST_START_MONITORING && (check_pid_monitored(syscall, (pid_t) pid) == 1)) {
		return -EBUSY;
	}
	...
	switch (cmd) {
		...
		case REQUEST_START_MONITORING:
			return request_start_monitoring(pid, syscall);
		case REQUEST_STOP_MONITORING:
			return request_stop_monitoring(pid, syscall);
	}
	return -EINVAL;
}
...
static int init_function(void) {
	...
	// ISO C90 warning
	int syscall;
	// hijack my_custom_syscall in orig_custom_syscall
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	
	// hijack exit_group syscall in orig_exit_group
	orig_exit_group = sys_call_table[__NR_exit_group];

	// lock sys call table
	// enable write and overwrite syscalls with methods
	
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long)sys_call_table);
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;
	sys_call_table[__NR_exit_group] = my_exit_group;
	//enable ready only / disable write
	set_addr_ro((unsigned long)sys_call_table);
	spin_unlock(&sys_call_table_lock);

	// lock my table
	// initialize struct for all syscalls
	spin_lock(&my_table_lock);
...
{   
	...
	// lock my table
	// initialize struct info for all syscalls
	spin_lock(&my_table_lock);
	for (syscall = 1; syscall < NR_syscalls; syscall++) {
		// reset intercepted syscalls to normal
